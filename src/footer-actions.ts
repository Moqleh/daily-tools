...
function enhanceFooter(){
 ...
 const legalBox=[...footer.querySelectorAll('div')].find(x=>x.querySelector('strong')?.textContent?.includes(isAr()?'الخصوصية':'Privacy')) as HTMLElement|undefined;
 if(legalBox){
  legalBox.classList.add('dt-legal-nav');
  legalBox.style.setProperty('display','flex','important');
  legalBox.style.setProperty('flex-direction','row','important');
  legalBox.style.setProperty('flex-wrap','nowrap','important');
  legalBox.style.setProperty('align-items','center','important');
  legalBox.style.setProperty('justify-content','center','important');
  legalBox.style.setProperty('gap','12px','important');
  legalBox.style.setProperty('width','100%','important');
  legalBox.style.setProperty('grid-column','1 / -1','important');
  legalBox.innerHTML='';
  ([['privacy','الخصوصية','Privacy'],['terms','الشروط','Terms'],['disclaimer','إخلاء المسؤولية','Disclaimer']] as [LegalKey,string,string][]).forEach(([key,a,e])=>{
   ...
   b.type='button';b.className='dt-legal-link';b.textContent=isAr()?a:e;b.onclick=()=>showLegal(key);
   ...
   legalBox.appendChild(b);
  });
 }
}

const css=document.createElement('style');css.textContent=`.dt-whatsapp{display:inline-flex;align-items:center;gap:10px;margin-top:8px;color:inherit;text-decoration:none;font-weight:800;direction:ltr}.dt-wa-icon{display:inline-grid;place-items:center;width:42px;height:42px;border-radius:50%;background:#25D366;color:#fff;box-shadow:0 6px 16px rgba(37,211,102,.28);transition:transform .18s ease,box-shadow .18s ease}.dt-wa-icon svg{width:25px;height:25px;display:block}.dt-wa-number{font-variant-numeric:tabular-nums;white-space:nowrap}.dt-whatsapp:hover .dt-wa-icon{transform:translateY(-2px) scale(1.04);box-shadow:0 9px 20px rgba(37,211,102,.34)}.dt-whatsapp:hover .dt-wa-number{text-decoration:underline}.dt-whatsapp:focus-visible{outline:3px solid rgba(37,211,102,.35);outline-offset:3px;border-radius:8px}footer .dt-legal-nav{grid-column:1/-1!important;width:100%!important;display:flex!important;flex-direction:row!important;align-items:center!important;justify-content:center!important;gap:12px!important;flex-wrap:nowrap!important}footer .dt-legal-link{display:inline-flex!important;align-items:center!important;justify-content:center!important;width:auto!important;min-width:150px!important;min-height:42px!important;margin:0!important;padding:9px 18px!important;border:1px solid rgba(47,117,181,.22)!important;border-radius:12px!important;background:rgba(255,255,255,.62)!important;color:inherit!important;font:inherit!important;font-weight:800!important;cursor:pointer!important;white-space:nowrap!important;flex:0 0 auto!important;transition:transform .18s ease,background .18s ease,border-color .18s ease}footer .dt-legal-link:hover{transform:translateY(-1px);background:rgba(238,247,255,.95)!important;border-color:rgba(47,117,181,.38)!important}.dt-legal-link:focus-visible{outline:3px solid rgba(47,117,181,.2);outline-offset:2px}.dt-legal-back{position:fixed;inset:0;z-index:10000;background:rgba(8,31,51,.58);display:grid;place-items:center;padding:20px}.dt-legal{position:relative;width:min(92vw,650px);background:#fff;color:#18384f;border-radius:22px;padding:30px;box-shadow:0 24px 70px rgba(0,0,0,.25);line-height:1.9}.dt-legal h2{margin:0 0 14px;color:#174f78}.dt-legal p{margin:0}.dt-legal-close{position:absolute;top:10px;inset-inline-end:14px;border:0;background:none;font-size:30px;cursor:pointer;color:#587487}@media(max-width:560px){footer .dt-legal-nav{gap:6px!important;overflow-x:auto!important;justify-content:flex-start!important;padding-bottom:4px!important}footer .dt-legal-link{min-width:126px!important;padding:7px 10px!important;font-size:12px!important}}`;
